...
const PUNGSU = {
  목: {
    ...
    rows: [
      { label: '주거 형태', text: '마당이나 정원이 있어 지기(地氣)를 직접 받을 수 있는 구조가 이상적입니다. 높이는 너무 낮아 습기가 차거나 너무 높아 기운이 허(虛)해지지 않도록, 나무가 자라는 높이처럼 중간 위치가 목형 기운과 잘 맞습니다.' },
  ...
  화: {
    ...
    rows: [
      { label: '주거 형태', text: '채광이 뛰어나고 시야가 트인 구조가 잘 맞습니다. 형기론에서 화형 건물은 위로 솟은 형태를 좋게 봅니다. 햇빛이 하루 종일 드는 높은 위치일수록 양기(陽氣)가 강해져 화의 기운이 살아납니다. 주택이라면 옥상이나 마당에 햇빛이 풍부한 구조, 공동주거라면 높은 층일수록 유리합니다.' },
  ...
  토: {
    ...
    rows: [
      { label: '주거 형태', text: '땅에 가깝고 넓게 퍼진 구조가 이상적입니다. 마당이나 정원이 있어 지기(地氣)를 직접 받을 수 있는 공간이 있으면 더욱 좋습니다. 높이는 낮을수록 안정적인 기운을 얻으며, 지하는 음기가 과도해 피하는 것이 좋습니다. 공동주거에서는 저층이, 주택이나 빌라에서는 지면과 가까운 구조가 토형 기운과 잘 맞습니다.' },
  ...
  금: {
    ...
    rows: [
      { label: '주거 형태', text: '구조가 정돈되고 관리가 체계적인 환경이 잘 맞습니다. 형기론에서 금형 건물은 돔이나 반원형처럼 둥글고 단정한 외관을 좋게 봅니다. 높이는 너무 높지도 낮지도 않은 중간이 이상적입니다. 너무 높으면 기운이 분산되고, 너무 낮으면 토기운에 눌릴 수 있어, 하늘과 지면의 기운이 균형 잡히는 중간 높이가 금의 기운을 가장 잘 살립니다.' },
  ...
  수: {
    ...
    rows: [
      { label: '주거 형태', text: '풍수에서 득수(得水), 즉 물을 얻는 것이 수 기운을 살리는 핵심입니다. 강변·하천 근처나 물이 보이거나 흐르는 소리가 들리는 환경을 우선 선택하세요. 특별한 높이 제약은 없으나, 물이 내려다보이는 시야가 확보되는 위치라면 기운을 관조하는 수의 성질과 더욱 잘 맞습니다.' },
  ...
  balanced: {
    ...
    rows: [
      { label: '주거 형태', text: '어느 형태든 무방하나, 공간의 역할이 명확하게 분리된 구조가 좋습니다. 풍수에서 기운이 방 용도에 따라 자연스럽게 구분되는 구조가 오행 균형을 유지하는 데 도움이 됩니다. 높이에 대한 특별한 제약도 없으며, 채광과 통풍이 고루 확보되는 위치라면 어떤 주거 형태에서도 오행의 균형이 자연스럽게 지원됩니다.' },
...
function genBoostText(counts) {
  ...
  html += envKeys.map(envKey => {
    ...
    const needNote    = cCount === 0
      ...
      : cCount === 1
      ? `<div class="boost-env-need">현재 사주에 <strong>${OH_NAME[cKey]}</strong>${cParticle} 약해 더욱 신경 쓰세요.</div>`
      : '';
